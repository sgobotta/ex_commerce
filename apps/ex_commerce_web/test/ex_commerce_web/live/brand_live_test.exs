defmodule ExCommerceWeb.BrandLiveTest do
  ...
  describe "Index" do
    setup [:register_and_log_in_confirmed_user, :create_brand]
    ...
    test "deletes brand in listing", %{conn: conn, brand: brand} do
      ...
      assert index_live
             |> element("#brand-#{brand.id} a", "Delete")
             |> render_click()
  ...
  describe "Show" do
    ...
    test "displays brand", %{conn: conn, brand: brand} do
      {:ok, _show_live, html} =
        live(conn, Routes.brand_show_path(conn, :show, brand))
