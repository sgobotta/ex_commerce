defmodule ExCommerce.Checkout do
  ...
  def get_order_message(%Cart{} = cart) do
    ...
    %Order{address: address, order_items: order_items, note: note} =
    ...
    order_items =
      Enum.reduce(order_items, "", fn %OrderItem{
                                      ...
                                      acc ->
        option_groups_message = get_option_groups_message(order_item)

        acc <>
          """
          ▪ (*#{quantity}*) #{name}, #{type} ... $#{variant_price}* (u) $#{order_item_price}*%0A
  ...
  defp get_option_groups_message(%OrderItem{
