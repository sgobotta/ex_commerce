defmodule ExCommerceWeb.QrCodeLive.Index do
  ...
  def render(assigns) do
    ...
    <div class="dashboard-container">
      ...
      <div id="qr_codes">
        <%= for %Shop{brand_id: brand_id, catalogues: catalogues, id: shop_id, name: name} <- @brand.shops do %>
          <div class="my-4">
          ...
          <div class="grid grid-cols-2 grid-flow-row gap-4">
            <div class="justify-self-center">
              <div class="flex justify-center">
                <div class="card p-2">
                  <.qr_code content={generate_shop_url(brand_id, shop_id)} color={<<255,0,0>>} width={100} />
                </div>
              </div>
            ...
            <%= for %Catalogue{id: catalogue_id, name: name} <- catalogues do %>
              <div class="justify-self-center">
                <div class="flex justify-center">
                  <div class="card p-2">
                    <.qr_code content={generate_catalogue_url(brand_id, shop_id, catalogue_id)} color={<<255,0,0>>} width={100} />
                  </div>
                </div>
                <div class="inline-flex items-center">
                  <div>
                  ...
                  <div class="pb-2">
                    <span class="text-base font-normal"><%= name %></span>
                  </div>
                </div>
              </div>
            <% end %>
  ...
  defp qr_code(assigns) do
    img_svg =
      QRCodeEx.encode(assigns.content)
      |> QRCodeEx.svg(width: 100, height: 100)

    assigns = assign(assigns, :img_svg, img_svg)
